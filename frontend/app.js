...
// Cache-busting for lang/<code>.json (fetched lazily by ensureLanguageLoaded,
// below). These files aren't referenced from app.html so its ASSET_VERSION
...
function set(p) {
  ...
  if (p.language) { applyLang(); ensureLanguageLoaded(p.language); }
...
function radar(being, becoming, connecting) {
  ...
  return '<svg width="100%" viewBox="0 0 300 250" style="display:block;max-width:240px;margin:0 auto;overflow:visible">'
    ...
    + '<text x="'+lb[0]+'" y="'+(lb[1]-14)+'" text-anchor="middle" font-size="12" font-weight="700" fill="var(--wave)" font-family="sans-serif">'+T('common.category_being')+'</text>'
    + '<text x="'+lb[0]+'" y="'+(lb[1]-1)+'"  text-anchor="middle" font-size="10" fill="rgba(0,229,200,0.7)" font-family="sans-serif">'+bPct+'%</text>'
    + '<text x="'+(lbc[0]+14)+'" y="'+(lbc[1]-8)+'" text-anchor="start" font-size="12" font-weight="700" fill="#6AB0FF" font-family="sans-serif">'+T('common.category_becoming')+'</text>'
    + '<text x="'+(lbc[0]+14)+'" y="'+(lbc[1]+6)+'" text-anchor="start" font-size="10" fill="rgba(106,176,255,0.7)" font-family="sans-serif">'+bcPct+'%</text>'
    + '<text x="'+(lc[0]-8)+'" y="'+lc[1]+'" text-anchor="end" font-size="12" font-weight="700" fill="var(--gold)" font-family="sans-serif">'+T('common.category_connecting')+'</text>'
  ...
  <div class="auth-wrap">
    <div class="auth-card">
      ...
      <div class="auth-switch">
        ${T('auth.new_here')} <a onclick="set({authMode:'register',authError:''})">${T('auth.create_account')}</a>
      </div>
      ` : `
      <div class="form-group">
        <label class="form-label">${T('common.email')}</label>
      ...
      <div class="auth-switch">
        ${T('auth.already_riding')} <a onclick="set({authMode:'login',authError:''})">${T('auth.sign_in')}</a>
      </div>
      `}
    </div>
...
const LANG = {  // Only English ships inline; other languages load
               // on demand from lang/<code>.json (see ensureLanguageLoaded).
  en: {
    ...
    'auth.already_riding':'Already riding?',
    ...
    'bio.char_count_hint':'(20–2000 characters)',
    ...
    'bio.switch_to_table':'Switch to Table →',
    ...
    'peripatos.the_companion':'The Companion',
...
// Only English ships inline in app.js (see LANG above). Every other
// language lives in its own file under lang/<code>.json and is fetched
// only the first time it's actually needed -- once for a returning user
// whose saved preference isn't English, or the moment someone switches
// away from English. Loaded packs are cached in memory for the session;
// LANG_LOADED tracks 'loading' to de-dupe concurrent requests.
var LANG_LOADED = { en: true };
function ensureLanguageLoaded(code) {
  if (!code || code === 'en' || LANG_LOADED[code] === true) return Promise.resolve();
  if (LANG_LOADED[code] === 'loading') return Promise.resolve();
  LANG_LOADED[code] = 'loading';
  return fetch('lang/' + code + '.json?v=' + LANG_ASSET_VERSION, { credentials: 'same-origin' })
    .then(function(res) {
    ...
    .then(function(data) {
      LANG[code] = data;
      LANG_LOADED[code] = true;
      draw(); // re-render now that the real translations are available
    })
    .catch(function(e) {
      console.warn('Failed to load language pack:', code, e);
      LANG_LOADED[code] = false; // allow a retry on the next attempt
    });
}
...
function phaseBody(p) {
  ...
  if(false && p===0) return `
    ...
    <div style="display:flex;gap:7px;flex-wrap:wrap">
      ...
      <span class="pill pw">${T('common.grow')}</span>
...
function renderSandboxThread(inputId, phaseLabel, showGenerateBtn) {
  ...
  if (S.sandboxLoading) {
    threadHTML += '<div style="display:flex;gap:8px;align-items:center;padding:4px 0">'
      ...
      + '<div style="font-size:12px;color:var(--text3);font-style:italic">'+T('session.thinking')+'</div></div>';
  }
  ...
  <div class="page">
    ...
    <div class="card card-wave" style="margin-bottom:20px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
        ...
        <span style="font-family:var(--font-display);font-size:14px;font-weight:700;color:var(--wave)">${T('peripatos.the_companion')}</span>
...
window.showPreferences = function() {
  ...
  modal.innerHTML =
    ...
    + '<div style="margin-bottom:18px">'
    + '<label style="font-size:12px;color:var(--text2);display:block;margin-bottom:8px">'+T('common.avatar')+'</label>'
...
function BioregionPage() {
  ...
  if (S.bioregionContribLoaded) {
    ...
    if (c) {
      ...
      myCard = '<div style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);padding:16px;margin-bottom:20px">'
        + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">'
        + '<div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.08em">'+T('bio.your_contribution')+'</div>'
        ...
        + (c.status==='pending' ? '<div style="margin-top:12px"><button onclick="withdrawBioregionContrib()" style="font-size:11px;padding:5px 14px;background:transparent;border:1px solid var(--border2);color:var(--text3);border-radius:20px;cursor:pointer">'+T('common.withdraw')+'</button></div>' : '')
        + '</div>';
      ...
      if (S.bioregionDraftLoading) {
        ...
        myCard = '<div style="background:linear-gradient(135deg,var(--bg2) 0%,rgba(0,229,200,0.04) 100%);border:1px dashed rgba(0,229,200,0.3);border-radius:var(--r);padding:20px;margin-bottom:20px;text-align:center">'
          + '<div style="font-size:22px;margin-bottom:10px">🌱</div>'
          + '<div style="font-size:14px;font-weight:600;color:var(--text1);margin-bottom:6px">'+T('bio.you_know_your_land')+'</div>'
  ...
  if (S.bioregionForm) {
    formHtml = '<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:20px;margin-bottom:20px">'
      ...
      + '</div>'
      + '<div style="margin-bottom:14px">'
      + '<label style="font-size:11px;font-weight:600;color:var(--text3);display:block;margin-bottom:5px">'+T('bio.your_statement')+' <span style="font-weight:400;color:var(--text3)">'+T('bio.char_count_hint')+'</span></label>'
    ...
    listHtml = '<div class="bioregion-grid">'
      + S.bioregionPortraits.map(function(p) {
          ...
          var distHtml = (p.distance_km != null)
            ? (p.distance_km <= 50
                ? ' · <span style="color:var(--wave)">'+T('bio.near_you')+'</span>'
...
function initBioregionMap(portraits) {
  ...
  if (!_bioHasWebGL()) {
    ...
    if (c) c.innerHTML = '<div class="bio-map-fallback"><p>'+T('bio.err_webgl_required')+'</p><button onclick="window.setBioView(\'table\')">'+T('bio.switch_to_table')+'</button></div>';
    return;
  }
...
function _renderBioTableView() {
  ...
  return '<div class="bio-tbl-wrap">'
    ...
    + '<th class="bio-th" scope="col"><span style="position:absolute;width:1px;height:1px;overflow:hidden">'+T('common.actions')+'</span></th>'
    + '</tr></thead>'
...
// ── Boot ──────────────────────────────────────────
async function boot() {
  // Fire off the saved UI-language pack fetch in the background (no-op if
  // it's English). Doesn't block first paint -- draw() below renders
  // immediately with the English fallback text, then re-renders once the
  // real pack lands a moment later, for returning non-English learners.
  ensureLanguageLoaded(S.language);
